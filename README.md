...
На Windows откройте папку проекта в Проводнике, кликните правой кнопкой по
пустому месту и выберите `Открыть в Терминале` / `Open in Terminal`.
...
2. Положить в корень проекта два файла от владельца проекта:
...
пароли приложений для двух Yandex участников.
...
Эти файлы создаются локально командами `auth-finish` и `live-links --prepare`.
